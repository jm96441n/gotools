...
import (
	...
	"github.com/jm96441n/gotools/pipeline"
)
...
func TestFromFileInvalid(t *testing.T) {
	t.Parallel()
	p := pipeline.FromFile("testdata/does-not-exist.txt")
	if p.Error == nil {
		t.Error("Expected error from reading non existent file, got nil")
	}
}
...
func TestStringError(t *testing.T) {
	...
	_, err := p.String()
	if err == nil {
		t.Error("expected error to be non-nil, but it was nil")
	}
}
...
func TestColumnInvalid(t *testing.T) {
	...
	err := p.Error
	if err == nil {
		t.Error("expected an error from setting a negative column value, got nil")
	}
}
